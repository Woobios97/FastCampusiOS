      ...
      VStack(spacing: 0) {
        if !todoListViewModel.todos.isEmpty {
          CustomNavigationBar(
            ...
            rightBtnType: todoListViewModel.navigationBarRightMode
          )
        } else {
          Spacer()
            .frame(height: 20)
...
private struct AnnouncementView: View {
  fileprivate var body: some View {
    VStack(spacing: 15) {
      ...
      Image("pencil")
        .renderingMode(.template) // Noti: 추후에 색상을 텍스트와 함께 변환시키기위함
      Text("\"매일 아침 5시 운동하자! 제발!\"")
      Text("\"내일 8시 수강 신청하자!\"")
      Text("\"1시 반 점심약속 리만이드 쫌!\"")
